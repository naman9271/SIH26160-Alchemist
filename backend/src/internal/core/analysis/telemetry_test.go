...
func TestPacketEvidenceNeverPromotesAnOfferToNegotiatedCrypto(t *testing.T) {
	offer := capture.PacketMetadata{SessionID: "s", IKE: true, IKEVersion: "IKEv2", IKEProposals: []capture.IKEProposal{{Number: 1, ProtocolID: 1, Transforms: []capture.IKETransform{{Type: 1, Name: "AES-CBC-128"}}}}, SeenAt: time.Now()}
	...
	if !hasEvidence(packetEvidence(selected), "ike.encryption") {
