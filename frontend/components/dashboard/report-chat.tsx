...
export function ReportChat() {
  ...
  async function send() {
    ...
    try {
      const report = compactReportForChat(items.find(v => v.id === selected)?.data);
      const response = await fetch("/api/report-chat", {method: "POST", headers: {"content-type": "application/json"}, body: JSON.stringify({report, messages: next.slice(-20)})});
