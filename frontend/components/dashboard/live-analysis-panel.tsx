...
type Report = { report_id: string; type?: string; format?: string; state: string; download_url?: string; failure_reason?: string };
...
function DataBlock({ title, value }: { title: string; value: unknown }) {
  ...
  return <details className="relative border border-white/15 bg-black/20 p-4" open={title === "Analysis summary" || title === "Pipeline progress"}>
    <span className="absolute right-4 top-3"><InfoHint label={`About ${title}`}>This panel displays the available evidence for {title.toLowerCase()}. Its source and evidence status determine whether it was observed passively, verified by an authorized gateway, derived, inferred, or not evaluated.</InfoHint></span>
    <summary className="cursor-pointer pr-8 text-[10px] font-bold tracking-[.13em] text-teal-100">{title.toUpperCase()} <span className="ml-2 text-white/45">{Array.isArray(value) ? `${value.length} RECORDS` : "AVAILABLE"}</span></summary>
...
export function LiveAnalysisPanel({ view = "overview" }: { view?: string }) {
  ...
  async function generateReport(type: "EXECUTIVE" | "TECHNICAL" = "EXECUTIVE") {
    if (!analysisId || report?.state === "GENERATING") return;
    try {
      setError(undefined);
      setReport({ report_id: "", state: "GENERATING" });
      setReport(await request<Report>(`/api/v1/analyses/${encodeURIComponent(analysisId)}/report`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ type }) }));
    }
