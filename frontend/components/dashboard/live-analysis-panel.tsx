...
const sectionsForView: Record<string, Array<[string, string[]]>> = {
  overview: [["Analysis summary", ["summary"]], ["Security posture", ["security", "assessment"]], ["Risk score", ["security", "risk_score"]], ["Fusion coverage", ["fusion", "summary"]], ["Capture quality", ["capture_quality"]]],
...
export function LiveAnalysisPanel({ view = "overview" }: { view?: string }) {
  ...
  return <section className="mt-7 border border-white/15 bg-white/[.025] p-5 sm:p-6">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-[.14em] text-sky-200">ANALYSIS DATA</p><p className="mt-2 text-xs text-white/55">Results retain evidence provenance. Offered suites, observed selections, gateway facts and ML inferences have different meanings.</p></div><button onClick={() => void refresh()} className="border border-white/30 px-3 py-2 text-[9px] font-bold tracking-[.12em] transition hover:bg-white hover:text-black">REFRESH</button></div>
