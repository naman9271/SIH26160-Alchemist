...
export function GuidePage() {
  return <section className="mt-7 space-y-8">
    <div className="border border-teal-200/25 bg-teal-200/[.035] p-6 sm:p-8">
      ...
      <p className="mt-5 max-w-3xl text-sm leading-7 text-white/65">IPSEC PRISM analyses observable IPsec protocol and traffic metadata. It keeps direct observations, deterministic results, ML inference, and authorized gateway verification separate. ESP payloads remain encrypted throughout.</p>
      <Link href="/workspace" className="group relative mt-6 inline-flex overflow-hidden border border-white/70 bg-white/10 px-4 py-2.5 text-[10px] font-bold tracking-[.12em] text-white shadow-[0_0_28px_rgba(255,255,255,.12)] transition-all duration-300 hover:border-teal-200 hover:bg-teal-200/15 hover:text-teal-100 hover:shadow-[0_0_38px_rgba(94,234,212,.28)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-200"><span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-teal-100/25 to-transparent transition-transform duration-700 group-hover:translate-x-full" /><span className="absolute -left-1 -top-1 hidden h-3 w-3 border-l border-t border-white/70 group-hover:border-teal-100 lg:block" /><span className="absolute -bottom-1 -right-1 hidden h-3 w-3 border-b border-r border-white/70 group-hover:border-teal-100 lg:block" /><span className="relative">START ANALYSIS</span></Link>
    </div>
