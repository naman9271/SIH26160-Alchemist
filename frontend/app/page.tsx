...
export default function Home() {
  ...
  async function start() {
    if (!upload) return;
    const useML = enableML && health?.dependencies.ml === "READY";
    setBusy(true); setMessage(useML ? "Starting passive PCAP and ML analysis…" : "Starting passive PCAP analysis…");
    try { const created = await api<{ analysis_id: string }>("/v1/analyses", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ source_id: upload.source_id, enable_ml: useML }) }); await refreshAnalysis(created.analysis_id); setMessage("Analysis is running. Results update automatically."); } catch (error) { setMessage(error instanceof Error ? error.message : "Analysis could not start."); } finally { setBusy(false); }
  }
  ...
  const analysisMessage = complete ? "Analysis completed. Results are final." : analysis?.state.includes("FAILED") ? "Analysis failed." : message;
