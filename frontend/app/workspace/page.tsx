...
const workspaceSteps = [
  ...
  ["02", "SOURCE", "Attach a PCAP or PCAPNG capture file."],
...
export default function WorkspacePage() {
  ...
  async function startWorkflow(event: FormEvent<HTMLFormElement>) {
    ...
    if (!file) return;
    if (!/\.(pcap|cap|pcapng)$/i.test(file.name)) {
      setError("Use a .pcap, .cap or .pcapng capture file.");
      return;
    }
    setError(undefined);
    setPhase("uploading");
      ...
      <div className="workspace-sidebar-shell mx-auto grid max-w-[1600px]">
        ...
        <main className="min-w-0 px-5 py-16 lg:px-8 lg:py-24">
          <div className="max-w-3xl border-l border-dashed border-white/40 pl-5">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-white/65">Upload PCAP or PCAPNG for passive analysis. Mode, ESP cipher, PFS and replay configuration may require authorized gateway evidence; unavailable facts remain unevaluated.</p>
          </div>
          ...
          <div id="workspace-input" className="mt-14 grid scroll-mt-24 border-l border-t border-white/20 lg:grid-cols-[1.3fr_.7fr]">
            ...
            {mode === "pcap" ? <>
            <p className="mt-7 text-[10px] tracking-[.16em] text-white/45">SOURCE / PCAP, CAP OR PCAPNG</p>
            <label className={`group relative mt-6 block cursor-pointer overflow-hidden border p-7 transition duration-300 sm:p-9 ${file ? "border-teal-200/70 bg-teal-200/[.055]" : "border-dashed border-white/35 bg-white/[.018] hover:border-teal-200/70 hover:bg-teal-200/[.04]"}`} htmlFor="pcap-upload">
              <input id="pcap-upload" className="sr-only" type="file" accept=".pcap,.cap,.pcapng,application/vnd.tcpdump.pcap" onChange={selectFile} />
              ...
              <span className="relative z-10 grid gap-6 sm:grid-cols-[auto_1fr] sm:items-center">
                ...
                <span>
                  ...
                  <span className="mt-3 block max-w-xl text-xs leading-6 text-white/55">
                    {file ? `${(file.size / 1024 / 1024).toFixed(2)} MiB selected · ready for passive evidence analysis` : "Choose a packet capture. ESP payload remains sealed; only protocol metadata is analysed."}
                  </span>
                  <span className="mt-5 flex flex-wrap gap-2 text-[9px] font-bold tracking-[.12em]">
                    ...
                    <span className="border border-amber-200/30 px-2.5 py-1 text-amber-100/65">.PCAPNG</span>
